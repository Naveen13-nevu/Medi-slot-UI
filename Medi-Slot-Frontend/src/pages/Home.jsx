    ...
    <Box sx={{ minHeight: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', py: 8, px: 2, background: 'linear-gradient(135deg, #eef2ff 0%, #ffffff 100%)' }}>
      <Container maxWidth="md">
        <Typography variant="h3" fontWeight="bold" gutterBottom>
          Book Your <Box component="span" sx={{ color: 'primary.main' }}>Doctor Appointment</Box> in Seconds
        </Typography>
        <Typography variant="h6" color="text.secondary" sx={{ mb: 4 }}>
          Skip the waiting room. Find the right doctor, see real‑time availability, and book instantly.
        </Typography>

        {user ? (
          <Button component={Link} to={user.role === 'PATIENT' ? '/patient' : '/doctor'} variant="contained" size="large" sx={{ px: 6, py: 2, fontSize: '1.1rem' }}>
            Go to Dashboard
          </Button>
        ) : (
